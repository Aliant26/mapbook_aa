...
def show_user_details()->None:
    ...
    label_liczba_postow_szczegoly_obiektu_wartosc.configure(text=posty)
...
def edit_user():
    ...
    button_dodaj_uzytkownika.configure(text="Zapisz zmiany", command=lambda: update_user(i))
...
def update_user(i):
    ...
    #users[i].marker = map_widget.set_position(users[i].coordinates[0], users[i].coordinates[1], text=users[i])
    users[i].marker.delete()
    users[i].marker = map_widget.set_marker(users[i].coordinates[0], users[i].coordinates[1], text=users[i].imie)

    button_dodaj_uzytkownika.configure(text="Dodaj użytkownika", command=add_user)
...
ramka_szczegoly_obiektow.grid(row=1, column=0, columnspan=2, padx=50, pady=50)
...
# RAMKA FORMULARZ
label_forumlarz = Label(ramka_formularz, text="Formularz: ")
...
label_forumlarz.grid(row=0, column=0, columnspan=2)
...
button_dodaj_uzytkownika = Button(ramka_formularz, text = "Dodaj znajomego", command = add_user)
...
label_liczba_postow_szczegoly_obiektu_wartosc = Label(ramka_szczegoly_obiektow, text = "...", width = 10)
...
label_imie_szczegoly_obiektu.grid(row=1, column=1, sticky=W)
